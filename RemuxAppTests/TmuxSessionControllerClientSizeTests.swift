...
final class TmuxSessionControllerClientSizeTests: XCTestCase {
    ...
    func testPaneNavigationCoalescesToLatestIntentAgainstFreshTopology() async throws {
        ...
        XCTAssertEqual(
            ...
            ["select-pane -Z -t %2\n"],
            "the deferred pane must be admitted once after fresh topology, in the pump drain"
        )
        ...
        XCTAssertEqual(
            harness.recorder.takeStrings(),
            ["select-pane -Z -t %2\n"],
            "A → B → A must restore the latest explicit intent"
        )
    ...
    func testPaneNavigationWaitsWhenCommandEndsBeforeTopology() async throws {
        ...
        XCTAssertEqual(
            harness.recorder.takeStrings(),
            ["select-pane -Z -t %0\n"],
            "fresh topology must admit exactly the latest deferred navigation"
        )
    ...
    func testPaneNavigationUsesTopologyThatArrivedBeforeCompletion() async throws {
        ...
        XCTAssertEqual(
            harness.recorder.takeStrings(),
            ["select-pane -Z -t %2\n"],
            "completion must recognize topology published since command submission"
        )
        await shutDown(harness.controller)
    }
    ...
    func testPaneNavigationWaitsWhenSubmittedAfterCompletionBeforeTopology() async throws {
        ...
        XCTAssertEqual(
            harness.recorder.takeStrings(),
            ["select-pane -Z -t %2\n"],
            "fresh topology must release exactly the latest navigation"
        )
        await shutDown(harness.controller)
    }
    ...
    func testPaneNavigationReevaluatesImmediatelyAfterCommandError() async throws {
        ...
        XCTAssertEqual(
            harness.recorder.takeStrings(),
            ["select-pane -Z -t %2\n"],
            "a rejected mutation cannot strand the latest valid navigation"
        )
        await shutDown(harness.controller)
    }
    ...
    func testRepeatedCrossWindowUnzoomedSelectionDoesNotQueueSecondToggle() async throws {
        ...
        XCTAssertEqual(
            harness.recorder.takeStrings(),
            ["select-window -t @1 ; resize-pane -Z -t %1\n"]
        )
    ...
    private func readyController(
        ...
        let hydration = (firstHydrationCommandNumber..<(firstHydrationCommandNumber + hydrationCommandCount))
            .map { "%begin \($0) \($0) 1\n%end \($0) \($0) 1\n" }
            .joined()
        controller.pump(Data(hydration.utf8))
        await drain(controller)
        ...
        return ReadyControllerHarness(
            ...
            recorder: recorder,
            nextCommandNumber: firstHydrationCommandNumber + hydrationCommandCount
        )
    }
